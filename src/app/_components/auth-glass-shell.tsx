...
export function AuthGlassShell({ title, subtitle, children }: AuthGlassShellProps) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 font-sans text-white antialiased px-6 py-12">
      ...
      {/* Dot grid */}
      <div className="pointer-events-none absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(rgba(255,255,255,0.03) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
      {/* Glassmorphic Card */}
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-slate-900/40 backdrop-blur-xl border border-white/10 shadow-2xl rounded-2xl p-8">
          ...
          {/* Title and subtitle */}
          <h1 className="text-2xl font-bold tracking-tight text-white text-center mb-2">{title}</h1>
          <p className="text-sm text-zinc-400 text-center mb-8">{subtitle}</p>
          {/* Form container */}
          <div>{children}</div>
        </div>
        <p className="mt-8 text-center text-xs text-zinc-600">© 2026 Kyvex · Made in Toronto 🍁</p>
      </div>
    </div>
  );
}
